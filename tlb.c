...
int main(int argc, char *argv[])
{
  if (argc < 3)
  {
    fprintf(stderr, "Usage: %s pages trails\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  if (pages < 1 || trails < 1)
  {
    fprintf(stderr, "Invalid input. Usage: %s pages trails\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  printf("%f\n", ((end.tv_sec - start.tv_sec) * 1e9 + (end.tv_nsec - start.tv_nsec)) / (trails * pages));
  free(a);
  return 0;
}